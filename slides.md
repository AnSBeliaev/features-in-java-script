...
- **Ломают внутреннюю оптимизацию движков.** Браузеру нужно заранее знать точную структуру класса, чтобы выполнять код быстро. Декоратор же меняет класс на лету — подменяет методы и задевает приватные поля. Структура класса перестаёт быть предсказуемой, и браузер выполняет такой код медленнее.
...
* **Предлагали добавить неизменяемые объекты и массивы.** Идея была в том, чтобы синтаксис `#{ a: 1 }` и `#[1, 2]` сравнивался по значениям внутри, а не по ссылке в памяти.
* **Проект полностью закрыли в апреле 2025 года.** После 6 лет разработки комитет официально отозвал предложение со второй стадии и отправил его в архив.
* **Глубокое сравнение перегружало браузеры.** Чтобы проверить равенство больших объектов через обычное `===`, движку приходилось бы тратить слишком много ресурсов процессора и памяти.
* **Заменяют проект на более простое решение.** Вместо тяжелых примитивов инженеры начали обсуждать **Composites**. Это будут специальные объекты, которые сравниваются через безопасный встроенный метод `Composite.equal()`.
